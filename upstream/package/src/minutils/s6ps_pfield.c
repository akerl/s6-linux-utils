...
static struct taia boottime = TAIA_EPOCH ;
...
int s6ps_compute_boottime (pscan_t *p, unsigned int mypos)
{
  ...
  else
  {
    ...
    struct taia offset = { .sec = { .x = p[mypos].start / hz }, .nano = (p[mypos].start % hz) * (1000000000 / hz), .atto = 0 } ;
    taia_sub(&boottime, &STAMP, &offset) ;
    return 1 ;
  }
}
...
static int fmt_timedate (pscan_t *p, unsigned int *pos, unsigned int *len, struct tm const *tm)
{
  ...
  unsigned int tmplen ;
  char *tmpstrf = "%F" ;
  if (!nowtm.tm_year && !localtm_from_tai(&nowtm, taia_secp(&STAMP), 1)) return 0 ;
...
static int fmt_start (pscan_t *p, unsigned int *pos, unsigned int *len)
{
  struct tm starttm ;
  unsigned int hz = gethz() ;
  struct taia blah = { .sec = { .x = p->start / hz }, .nano = (p->start % hz) * (1000000000 / hz), .atto = 0 } ;
  taia_add(&blah, &boottime, &blah) ;
  if (!localtm_from_tai(&starttm, taia_secp(&blah), 1)) return 0 ;
...
{
  struct taia totaltime ;
  register unsigned int hz = gethz() ;
  taia_sub(&totaltime, &STAMP, &boottime) ;
